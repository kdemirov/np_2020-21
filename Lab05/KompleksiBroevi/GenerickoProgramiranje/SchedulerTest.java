package Lab05.KompleksiBroevi.GenerickoProgramiranje;
...
class Timestamp<T> implements Comparable<Timestamp<T>> {
    ...
    public Timestamp(LocalDateTime date, T element) {
        this.date = date;
    ...
    public boolean equals(Object o) {
        ...
        return Objects.equals(date, timestamp.date);
    }
...
class Scheduler<T> {
    ...
    public boolean remove(Timestamp<T> timestamp) {
        return timestamps.removeIf(t -> t.equals(timestamp));
    }

    public Timestamp<T> next() {
        return timestamps.stream().filter(t -> t.getTime().isAfter(LocalDateTime.now())).sorted().findFirst().get();
    }

    public Timestamp<T> last() {

        return timestamps.stream().filter(t -> t.getTime().isBefore(LocalDateTime.now())).sorted(Comparator.reverseOrder()).findFirst().get();
