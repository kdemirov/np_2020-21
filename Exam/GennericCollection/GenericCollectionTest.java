package Exam.GennericCollection;
...
class GenericCollection<T extends Comparable<T> & IHasTimestamp>{
    private Map<String,Set<T>> elementsByCategory;

    public GenericCollection(){
        this.elementsByCategory=new TreeMap<>();
    }
    public void addGenericItem(String category ,T element){
        this.elementsByCategory.putIfAbsent(category,new TreeSet<>());
        this.elementsByCategory.get(category).add(element);
    }
    ...
    private String getMonthAndDay(LocalDateTime time){
        return String.format("%02d-%02d",time.getMonth().getValue(),time.getDayOfMonth());
    }
    public Map<String,Set<T>> byMonthAndDay(){
        return this.elementsByCategory
                ...
                .collect(Collectors.groupingBy(
                        t->this.getMonthAndDay(t.getTimestamp()),
                        TreeMap::new,
   ...
   public Collection<T> itemsFromCategories(List<String> categories){
        Set<T> newSet=new TreeSet<>(Comparator.reverseOrder());
        categories.forEach(c-> newSet.addAll(this.elementsByCategory.get(c)));
        return newSet;
   }
}
